...
app.use(async (ctx) => {
  try {
    ...
    let template, render: (url: string, manifest: Record<string, string[]>, cookie?: string, host?: string) => Promise<string[]>;
    if (!isProd) {
    ...
    const [appHtml, preloadLinks, metadata, initialState, status] = await render(
      url,
      manifest,
      cookie,
