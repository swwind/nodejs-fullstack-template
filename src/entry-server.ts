...
export async function render(url: string, manifest: Record<string, string[]>, cookie?: string, host?: string) {
  ...
  const metadata = `<title>${escapeHtml(title)}</title>` + Object.keys(meta).map((key) => {
    return `<meta name=${JSON.stringify(key)} content=${JSON.stringify(meta[key])}>`;
  }).join('');

  const initialState = `<script>window.__INITIAL_STATE__=${
    JSON.stringify(store.state).replace(/\//g, '\\/')};</script>`
